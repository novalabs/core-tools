...
from numpy.fft.fftpack import ihfft
...
class BootMsg(Message):
    ...
    class UID(Serializable):
        ...
        def __init__(self, _BootMsg, uid=''):
            super(BootMsg.UID, self).__init__()
            self._BootMsg = _BootMsg
            self.uid = uid

        def __repr__(self):
            return '%s::[uid=%s]' % ( type(self).__name__, ''.join(format(x, '02x') for x in self.uid))

        def marshal(self):
            return struct.pack('<%dB' % self.PAYLOAD_LENGTH, *self.uid)

        def unmarshal(self, data, offset=0):
            self.uid = struct.unpack_from('<%dB' % self.PAYLOAD_LENGTH, data, offset)

        @staticmethod
        def getUIDFromHexString(hex_string):
            return struct.unpack('<%dB' % BootMsg.UID.PAYLOAD_LENGTH, hex_string.decode('hex'))

    class ANNOUNCE(Serializable):
        UID_LENGTH = 12
        MODULE_TYPE_LENGTH = 16
        MODULE_NAME_LENGTH = 16
        PAYLOAD_LENGTH = UID_LENGTH + MODULE_TYPE_LENGTH + MODULE_NAME_LENGTH

        def __init__(self, _BootMsg, uid='', moduleType='', moduleName=''):
        ...
        def cleanString(self, str): # Maybe there is a better way to achive the same result. IDGAF!
            tmp = ''
            for c in str:
                if c != 0:
                    tmp = tmp + chr(c)
        ...
        def __repr__(self):
            return '%s::[uid=%s, module=%s, name=%s]' % (type(self).__name__, ''.join(format(x, '02x') for x in self.uid), self.moduleType , self.moduleName)

        def marshal(self):
            return struct.pack('<%dB' % self.PAYLOAD_LENGTH, *self.uid)

        def unmarshal(self, data, offset=0):
            self.uid = struct.unpack_from('<%dB' % self.UID_LENGTH, data, offset)
            self.moduleType = self.cleanString(struct.unpack_from('<%dB' % self.MODULE_TYPE_LENGTH, data, self.UID_LENGTH))
            self.moduleName = self.cleanString(struct.unpack_from('<%dB' % self.MODULE_NAME_LENGTH, data, self.UID_LENGTH + self.MODULE_TYPE_LENGTH))
    ...
    class Acknowledge(Serializable):
        ...
        def __init__(self, _BootMsg, status=AckEnum.NONE, cmd=0x00, uid=''):
            super(BootMsg.Acknowledge, self).__init__()
            self._BootMsg = _BootMsg
            self.uid = uid
            self.cmd = cmd
            self.status = status
        ...
        def __repr__(self):
            return '%s::[status=%s, cmd=%s, uid=%s]' % (type(self).__name__, BootMsg.Acknowledge.AckEnum._reverse[self.status], BootMsg.TypeEnum._reverse[self.cmd], ''.join(format(x, '02x') for x in self.uid))

        def marshal(self):
            return struct.pack('<BB%dB' % BootMsg.UID.PAYLOAD_LENGTH, self.status, self.cmd, *self.uid)  # marshal UID as it must be done

        def unmarshal(self, data, offset=0):
            self.status, self.cmd, tmp = struct.unpack_from('<BB%ds' % BootMsg.UID.PAYLOAD_LENGTH, data, offset) # unsmarshal UID as it must be done
            self.uid = struct.unpack_from('<%dB' % BootMsg.UID.PAYLOAD_LENGTH, tmp, 0)
    ...
    def __init__(self, cmd=TypeEnum.NONE, seq=0xFF):
        ...
        self.announce = BootMsg.ANNOUNCE(self)
    ...
    def __repr__(self):
        ...
        elif t in (e.BOOTLOAD,):
            subtext = repr(self.empty)
            subtype = type(self.empty).__name__
        elif t in (e.IDENTIFY_SLAVE, e.SELECT_SLAVE, e.DESELECT_SLAVE, e.ERASE_PROGRAM, e.ERASE_CONFIGURATION, e.ERASE_USER_CONFIGURATION, e.RESET, ):
        ...
        return '%s::[cmd=%s seq=%d pl=%s]' % (type(self).__name__, e._reverse[t],  self.seq, subtext)


    def marshal(self):
        cmd = self.cmd
        e = BootMsg.TypeEnum
        if cmd in (e.REQUEST, e.IDENTIFY_SLAVE, e.SELECT_SLAVE, e.DESELECT_SLAVE, e.ERASE_PROGRAM, e.ERASE_CONFIGURATION, e.ERASE_USER_CONFIGURATION, e.ACK, e.RESET):
            bytes = self.uid.marshal()
            length = self.uid.PAYLOAD_LENGTH
            padding = self.MAX_PAYLOAD_LENGTH - length
        elif cmd in (e.IHEX, ):
            bytes = self.ihex.marshal()
            length = self.ihex.PAYLOAD_LENGTH
            padding = self.MAX_PAYLOAD_LENGTH - length
        elif cmd in (e.BOOTLOAD, ):
            bytes = self.empty.marshal()
            length = self.empty.PAYLOAD_LENGTH
            padding = self.MAX_PAYLOAD_LENGTH - length
        else:
            raise ValueError('Unknown boot message command %d' % self.cmd)

        return struct.pack('<BB%ds%ds' % (length, padding), self.cmd, self.seq, bytes, padding * "\0")
    ...
    def unmarshal(self, data, offset=0):
        ...
        elif cmd in (e.IDENTIFY_SLAVE, e.SELECT_SLAVE, e.DESELECT_SLAVE, e.ERASE_PROGRAM, e.ERASE_CONFIGURATION, e.ERASE_USER_CONFIGURATION, e.RESET):
...
class Bootloader(object):
    ...
    def _callback(self, msg):
        ...
        if msg.cmd == BootMsg.TypeEnum.REQUEST:
            if self._slaves.has_key(msg.announce.uid):
                self._slaves[msg.announce.uid] += 1
            else:
                self._slaves[msg.announce.uid] = 1
                self._slavesTypes[msg.announce.uid] = msg.announce.moduleType
                self._slavesNames[msg.announce.uid] = msg.announce.moduleName
    ...
    def _rx(self, timeout = float(0.25)):
        try:
    ...
    def _emptyCommand(self, cmd, seq=None):
        ...
        self._tx(m)
        #time.sleep(1)
        if self.pollForAck(m) == BootMsg.Acknowledge.AckEnum.OK:
            return True
        else:
            print
            return False
    ...
    def _ihexCommand(self, cmd, type, ihex, seq=None):
        if seq is None:
            seq = self._lastSeq + 1
###            self._lastSeq = seq
            if seq == 256:
...
###        seq = self._lastSeq + 1
    ...
    def ihex(self, type, ihex):
        return self._ihexCommand(BootMsg.TypeEnum.IHEX, type, ihex)

    def waitForAck(self, m, timeout=0.5):
